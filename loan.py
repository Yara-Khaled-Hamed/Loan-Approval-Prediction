...
# Use the 'Set2' color palette, which has 8 colors by default
    ...
    st.write("This chart visualizes the **loan status distribution** for each loan grade in pie chart format.")

    # Number of columns for pie charts
    n_cols = 3
    ...
    for i, grade in enumerate(loan_grades):
        ...
        sizes = [data[status][i] for status in loan_statuses]  # Data for current grade
    ...
    age = st.sidebar.number_input("Age", min_value=20, max_value=100)
